...
A token is used to authorize a user when they're logging into a website. The token gets set to the local storage and tells the website that they are still logged in so that the user can see protected data.

- [x] What steps can you take in your web apps to keep your data secure?

You can add protected routes to pages that you want to only be seen by a user that has logged in. You can also send an auth header with API calls.

- [x] Describe how web servers work.

A web server stores code for websites so it can be shared to a computer that is accessing that website from where ever it is connected to the web. Computers access those sites through the URL.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

The PUT HTTP resource method and the DELETE HTTP request method.
